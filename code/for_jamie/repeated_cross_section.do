...
ssaggregate growth* decomp*_cgoodman_cz_pc n*_cgoodman_cz_pc GM_raw_pp [aw=popc1940], n("origin_fips") l(cz) sfile("shares_base.dta") controls("`b_controls' `extra_controls'") s(share)
...
graph export "es_by_year.png", as(png) replace
...
twoway 	(scatter b year, color(black)) ///
				...
				yline(0, lc(red) lp(dash)) ///
				legend(off) ytitle("Change in Municipalities P.C.")
graph export "es_by_year_pct.png", as(png) replace
...
graph export "es_by_year_decomp.png", as(png) replace
drop b ci_lo ci_hi year yearlab
...
graph export "es_by_year_popgrowth.png", as(png) replace
...
ssaggregate n*_cgoodman_cz_pc GM_raw_pp [aw=popc1940], n("origin_fips") l(cz) sfile("shares_base.dta") controls("`b_controls'") s(share)
...
graph export "event_study_pp_noctrl.png", as(png) replace
...
graph export "event_study_pct_noctrl.png", as(png) replace