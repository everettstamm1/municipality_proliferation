cap prog drop main_table_long_ssagg_nonpc
prog def main_table_long_ssagg_nonpc
	...
	foreach outcome in cgoodman schdist_ind gen_muni spdist totfrac {
		...
		// First Stage
		eststo fs_`outcome' : reg `endog' `exog' `ctrls' if !mi(n_`outcome'_cz) [aw=`weight'], r
		test `exog'=0 
		local F_`outcome' : di %6.2f r(F)

		// OLS 1940-70
		eststo ols70_`outcome' : reg n_`outcome'_cz `endog' `endog2' `ctrls' [aw = `weight'], r
		
			
		

		// OLS 1940-2010
		eststo ols10_`outcome' : reg ld_`outcome'_cz `endog' `endog2' `ctrls' [aw = `weight'], r
		local N_`outcome' = e(N)

	}
	
	preserve 
		ssaggregate n_cgoodman_cz n_totfrac_cz n_gen_muni_cz n_spdist_cz ld_cgoodman_cz ld_totfrac_cz ld_gen_muni_cz ld_spdist_cz `endog' [aw=`weight'], n(`origin_id') l(cz) sfile("`share_folder'/shares_`version'.dta") controls("`ctrls'") s(share)
		...
		foreach outcome in cgoodman  spdist gen_muni totfrac {
			eststo iv70_`outcome': ivreg2 n_`outcome'_cz (`endog' `endog2' = shift) [aw = s_n]
			
			estadd scalar dep_var70 = `dv70_`outcome''
			eststo iv10_`outcome': ivreg2 ld_`outcome'_cz (`endog' `endog2' = shift) [aw = s_n]
	...
	preserve 
		ssaggregate n_schdist_ind_cz ld_schdist_ind_cz `endog' [aw=`weight'], n(`origin_id') l(cz) sfile("`share_folder'/shares_`version'.dta") controls("`ctrls'") s(share)
