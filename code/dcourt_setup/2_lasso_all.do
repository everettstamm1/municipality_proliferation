...
*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------%
	...
	merge m:1 year countyicp state_abbrev using "$INTDATA/south_migrate", keep(1 3) nogen keepusing(pop_l netmig)
	drop if netmig == .
	...
	*drop if `var'==.
	...
	keep if year==1950
	drop year
	save "$INTDATA/dcourt/south_county_all_migration_dataset_for_prediction_1950.dta", replace
	restore
	
	preserve
	keep if year==1960
	drop year
	save "$INTDATA/dcourt/south_county_all_migration_dataset_for_prediction_1960.dta", replace
	restore
	
	preserve
	keep if year==1970
	drop year
	save "$INTDATA/dcourt/south_county_all_migration_dataset_for_prediction_1970.dta", replace
	restore
...
*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------%
	...
	rsource, terminator(END_OF_R) 
		...
		if (runLasso) {
		train = read_dta('south_county_all_migration_dataset_for_prediction_1950.dta')
	
		x = model.matrix(~., data=train %>% select(-netmig))
		y = train$netmig
	...
	write.csv(lasso_list_of_vars_1950[2:dim(lasso_list_of_vars_1950)[1],1:dim(lasso_list_of_vars_1950)[2]], file='lasso_list_of_vars_all_1950.csv')
	
		train = read_dta('south_county_all_migration_dataset_for_prediction_1960.dta')
	...
	write.csv(lasso_list_of_vars_1960[2:dim(lasso_list_of_vars_1960)[1],1:dim(lasso_list_of_vars_1960)[2]], file='lasso_list_of_vars_all_1960.csv')
	
		train = read_dta('south_county_all_migration_dataset_for_prediction_1970.dta')
	...
	write.csv(lasso_list_of_vars_1970[2:dim(lasso_list_of_vars_1970)[1],1:dim(lasso_list_of_vars_1970)[2]], file='lasso_list_of_vars_all_1970.csv')
	}
	END_OF_R
...
*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------%
	...
	use "$INTDATA/dcourt/clean_south_county_all.dta", clear
	
	/* Predict county-level net migration rate, decade by decade with southern 
	variables chosen by LASSO. Predict net migration rate ("netmig_pred") based on 
	these vars alone. */
	reg netmig percot perten perag peragtob tob warfac_pc permin perminot ot if year==1950
	predict netmig_pred if year==1950
	reg netmig percot perten perag peragtob tob warfac_pc permin perminot ot if year==1960
	predict netmig_pred01 if year==1960
	reg netmig percot perten perag peragtob tob warfac_pc permin perminot ot if year==1970
	predict netmig_pred02 if year==1970	
	
	replace netmig_pred=netmig_pred01 if year==1960
	replace netmig_pred=netmig_pred02 if year==1970
	drop netmig_pred01 netmig_pred02
	
	/* Boustan (2016): Total number leaving/coming to county: actual and predicted. Note 
	that netmig is a migration rate (per 100 residents). So, the range is -100 to 
	...
	gen totwmig=((pop_l/100)*netmig)
	gen totwmig_pred=((pop_l/100)*netmig_pred)
	gen weight=netmig_pred*pop_l
	...
	keep origin_fips year proutmig actoutmig netmig_pred
	
	drop if netmig_pred==. | proutmig==.
	...
	save "$INTDATA/dcourt/1_boustan_predict_mig_all.dta", replace
	
	collapse (sum) netmig_pred actoutmig proutmig, by(origin_fips year)
	save "$INTDATA/dcourt/1_boustan_predict_mig_collapsed_all.dta", replace
...
*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------%
	...
	use "$INTDATA/dcourt/clean_south_county_all.dta", clear
	g bshare_l = bpop_l / pop_l
	/* Predict county-level net migration rate, decade by decade with southern 
	variables chosen by LASSO. Predict net migration rate ("netmig_pred") based on 
	these vars alone.
	just drop tob in 1950 */

	reg netmig percot perten perag peragtob warfac_pc permin perminot ot if year==1950
	predict netmig_pred if year==1950
	reg netmig percot perten perag peragtob tob warfac_pc permin perminot ot if year==1960
	predict netmig_pred01 if year==1960
	reg netmig percot perten perag peragtob tob warfac_pc permin perminot ot if year==1970
	...
	*/
	...
	tostring origin_fips, replace
	keep origin_fips origin_state_name year proutmig actoutmig netmig_pred bshare_l
	...
	save "$INTDATA/dcourt/2_lasso_boustan_predict_mig_all.dta", replace
	restore
		
	replace proutmig = bshare_l * proutmig
	replace actoutmig = bshare_l * actoutmig
	keep origin_fips year proutmig actoutmig 
	save "$INTDATA/dcourt/2_lasso_boustan_predict_mig_all_bint.dta", replace
