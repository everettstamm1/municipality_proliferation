...
*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------%
	...
	*/
	...
	collapse (sum) netwmig_pred actoutmig proutmig, by(origin_state_name year)
...
*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------%
	...
	reg netwmig  if year==1950, absorb(stateicp)
	predict netwmig_resid if year==1950, resid
	reg netwmig  if year==1960, absorb(stateicp)
	predict netwmig_resid01 if year==1960, resid
	reg netwmig  if year==1970, absorb(stateicp)
	predict netwmig_resid02 if year==1970, resid

	replace netwmig_resid=netwmig_resid01 if year==1960
	replace netwmig_resid=netwmig_resid02 if year==1970
	drop netwmig_resid01 netwmig_resid02
	...
	gen totwmig=((wpop_l/100)*netwmig)
	gen totwmig_resid=((wpop_l/100)*netwmig_resid)
	gen weight=netwmig_resid*wpop_l
	...
	drop if countyfips==.
	rename totwmig actoutmig
	rename totwmig_resid residoutmig
	...
	tostring origin_fips, replace
	keep origin_fips year residoutmig actoutmig netwmig_resid
	
	drop if netwmig_resid==. | residoutmig==.
	...
	save "$INTDATA/dcourt/3_residstate_act_mig_white.dta", replace
	
	collapse (sum) netwmig_resid actoutmig residoutmig, by(origin_fips year)
	save "$INTDATA/dcourt//3_residstate_act_mig_collapsed_white.dta", replace	
...
*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------%
	...
	*drop if perurb > `r(p99)'
	...
	save "$INTDATA/dcourt/rur_boustan_predict_mig_white.dta", replace
	
	collapse (sum) netwmig_pred actoutmig proutmig, by(origin_fips year)
	save "$INTDATA/dcourt/rur_boustan_predict_mig_collapsed_white.dta", replace		
