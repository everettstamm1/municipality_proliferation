...
*/
	...
	gz7, filepath("$data/new_data") filename("usa_00048.dta.gz")
	...
	collapse (sum) popc bpopc (mean) citypop, by(city statefip)
	drop if city==0
	...
	*Merge with State Crosswalks
	...
	replace city = "Grosse Pointe Woods, MI" if city=="Grosse Pointe Park, MI" // One spot change

	save $population/clean_city_population_census_1940_full.dta, replace
