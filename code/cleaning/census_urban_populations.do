...
collapse (sum) popc, by(stateicp countyicp year)
...
foreach var of varlist popc{
	replace `var' = `var'*weight
}

collapse (sum) popc, by(year nhgisst_1990 nhgiscty_1990)
...
preserve 
	collapse (sum) popc, by(cz decade)
