...
*/
	...
	foreach group in $groups{
		...
		egen total_${weight}$origin_id = total(${weight}),by($origin_id)	
		egen total_${weight}$destination_id = total(${weight}), by($destination_id)
		
		
		keep if $dest_sample == 1
			
		/* Collapse data to destination county X origin county level */
		collapse (sum) ${weight} (mean) total_* , by($origin_id $destination_id)
		
		gen ${weight}$origin_id=${weight}/total_${weight}$origin_id
		
		/***** Save the origin-destination matrix as an intermediate dataset. *****/	
		save "$INTDATA/dcourt/bartik/${version}_od_matrix_${weight}_$start_year.dta", replace
...
*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------%
		...
		use "$INTDATA/dcourt/bartik/${version}_od_matrix_${weight}_$start_year.dta", clear
		
		/* Keep only necessary 
		vars for construction of matrix */
		
		keep ${weight}$origin_id $origin_id $destination_id total_${weight}$destination_id
		...
		reshape wide ${weight}$origin_id total_${weight}$destination_id, i($destination_id) j($origin_id) string

		foreach var of varlist ${weight}$origin_id*{
			replace `var'=0 if `var'==.
		}
		
		/***** Save wide version at destination level as dta file. *****/

		save "$INTDATA/dcourt/bartik/${version}_${weight}$origin_id$start_year.dta", replace

		drop total_${weight}$destination_id*

		/***** Save wide version at origin level as dta file. *****/
		
		reshape long ${weight}$origin_id, i($destination_id) j($origin_id) string
			
		reshape wide ${weight}$origin_id, i($origin_id) j( $destination_id) 
		
		save "$INTDATA/dcourt/bartik/${version}_wide_${weight}$origin_id$start_year.dta", replace
...
*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------%
		...
		foreach weight_type in $weight_types{ // Actual migration and predicted migration ("act" and "pr")
			...
			use "$INTDATA/dcourt/bartik/${version}_wide_${weight}$origin_id$start_year.dta", clear
			...
			reshape long shX`weight_type'_${weight}$origin_id, i($origin_id year) j($destination_id)
			reshape wide shX`weight_type'_${weight}$origin_id, i($destination_id year) j($origin_id) string
			
			rename *${weight}$origin_id* **
			...
			save "$INTDATA/dcourt/bartik/${version}_${weight}_`weight_type'$weight_var$origin_id$start_year`end_year'_all_wide.dta", replace
			keep $destination_id totshX`weight_type'*
			rename totshX* ${weight}_`weight_type'$weight_var*
			save "$INTDATA/dcourt/bartik/${version}_${weight}_`weight_type'$weight_var$origin_id$start_year`end_year'_collapsed_wide.dta", replace
		}
	}
