...
*/
	...
	foreach group in $groups{
		...
		save "data/interim/${version}_od_matrix_`group'_$start_year.dta", replace
...
*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------%
		...
		use "data/interim/${version}_od_matrix_`group'_$start_year.dta", clear
		...
		save "data/output/${version}_`group'$origin_id$start_year.dta", replace
		...
		save "data/interim/${version}_wide_`group'$origin_id$start_year.dta", replace
...
*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------%
		...
		foreach weight_type in $weight_types{ // Actual migration and predicted migration ("act" and "pr")
			...
			use "output/${version}_wide_`group'$origin_id$start_year.dta", clear
			...
			save "data/output/data/${version}_`group'_`weight_type'$weight_var$origin_id$start_year`end_year'_collapsed_wide.dta", replace
		}
	}
