...
esttab A using "$TABS/T1.tex", replace cells((mean(label("Mean") fmt(2)) p10(label("10th Percentile") fmt(2)) p50(label("Median") fmt(2)) p90(label("90th Percentile") fmt(2)))) booktabs compress frag label nonum noobs prehead( \begin{tabular}{l*{4}{c}} \toprule) posthead("\cmidrule(lr){1-5}" "\multicolumn{5}{l}{Panel A: Outcome Variables}\\" "\cmidrule(lr){1-5}" ) substitute("\_" "_")

eststo B: estpost summarize `panel_B_vars' , d

esttab B using "$TABS/T1.tex", append cells((mean(label(" ") fmt(2)) p10(label(" ") fmt(2)) p50(label(" ") fmt(2)) p90(label(" ") fmt(2)))) booktabs compress frag label nonum noobs nomtitle posthead("\cmidrule(lr){1-5}" "\multicolumn{5}{l}{Panel B: Treatment Variables}\\" "\cmidrule(lr){1-5}" ) mlabels(,none) collabels(,none) substitute("\_" "_")


eststo C: estpost summarize `panel_C_vars', d
esttab C using "$TABS/T1.tex", append  cells((mean(label(" ") fmt(2)) p10(label(" ") fmt(2)) p50(label(" ") fmt(2)) p90(label(" ") fmt(2)))) booktabs compress frag label nonum noobs nomtitle postfoot(\midrule Observations    &      130   &      130   &      130   &      130   \\	\bottomrule \end{tabular})  posthead("\cmidrule(lr){1-5}" "\multicolumn{5}{l}{Panel C: Control Variables}\\" "\cmidrule(lr){1-5}" ) mlabels(,none) collabels(,none) substitute("\_" "_")
