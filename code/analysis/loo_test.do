foreach outcome in cgoodman schdist_ind gen_subcounty spdist{
	...
	// RF
	reg n_`outcome'_cz_pcc GM_hat_raw_pp reg2 reg3 reg4 [aw=pop1940], r
	local b_rf = _b[GM_hat_raw_pp]
	local se_rf = _se[GM_hat_raw_pp]
	
	ivreg2 n_`outcome'_cz_pcc (GM_raw_pp = GM_hat_raw_pp) reg2 reg3 reg4 [aw=pop1940], r
	local b_iv = _b[GM_raw_pp]
	local se_iv = _se[GM_raw_pp]
	levelsof cz, local(czs)
	
	foreach cz in `czs'{
		qui parmby "reg n_`outcome'_cz_pcc GM_hat_raw_pp reg2 reg3 reg4 [aw=popc1940] if cz!=`cz', r", lab saving(`"rf`cz'`outcome'"', replace) idn(`cz') ids(vr) ylabel rename(idn vrsn) level(95 99)
		
		qui parmby "ivreg2 n_`outcome'_cz_pcc (GM_raw_pp = GM_hat_raw_pp) reg2 reg3 reg4 [aw=popc1940] if cz!=`cz', r", lab saving(`"iv`cz'`outcome'"', replace) idn(`cz') ids(vr) ylabel rename(idn vrsn) level(95 99)
