...
foreach outcome in  cgoodman_ gen_muni_ spdist_ schdist_ind_ totfrac_{
	...
	preserve
		...
		ssaggregate GM_raw_pp_ `outcome' [aw=popc1940], n("origin_fips") l(cz) sfile("$INTDATA/ssaggregate_prep/shares_panel_base.dta") controls("`b_controls' `extra_controls'") s(share) t(year)

		merge 1:1 origin_fips year using "$INTDATA/ssaggregate_prep/shock_instrument_panel_base.dta", keep(1 3) nogen
...
file open fh using "$TABS/RR2/panel_IV.tex", write replace
...
foreach outcome in  cgoodman_ gen_muni_ spdist_ schdist_ind_ totfrac_{
	...
	foreach y in 1940 1950 1960{
		preserve
			...
			ssaggregate GM_raw_pp_ `outcome' [aw=popc1940], n("origin_fips") l(cz) sfile("$INTDATA/ssaggregate_prep/shares_panel_`y'_base.dta") controls("`b_controls' `extra_controls' sumshare_split_") s(share) 

			merge 1:1 origin_fips using "$INTDATA/ssaggregate_prep/shock_instrument_panel_1940_base.dta", keep(1 3) nogen
			replace shift = 0 if mi(shift)
	...
	graph export "$FIGS/RR2/`outcome'split_IV.png", as(png) replace
