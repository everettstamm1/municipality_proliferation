	...
	*/
	...
	use  "$CLEANDATA/panel_a_data", clear

	bys dest_cz (cz_recent_black_migrants) : g t5 =  _n+5>=_N // keep top 5 sending counties
	bys origin_fips (t5) : replace t5 = t5[_N]
	bys dest_cz : egen totmig = total(cz_recent_black_migrants)
	keep if t5==1
	
	gsort dest_cz -cz_recent_black_migrants

	g order = _n if dest_cz==15200
	bys origin_fips (order) : replace order = order[1]+10 if dest_cz!=15200
	
	labmask order, values(origin_fips_name)
	g cz_recent_black_pct = cz_recent_black_migrants/totmig
	twoway bar cz_recent_black_pct order, ///
	caption("      Cleveland                                    Columbus          ", ring(0) position(12) size(large)) ///
	xlabel(1(1)20, valuelabel angle(45) labsize(small)) xline(10.5, lcolor(red) lpattern(dash))
