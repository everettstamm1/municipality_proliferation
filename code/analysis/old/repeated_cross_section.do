...
keep n_cgoodman_cz_pc cz growth* decomp* pop* popc1940 GM_raw_pp `b_controls' `extra_controls' n18* n19*_cgoodman_cz_pc n20*_cgoodman_cz_pc above_x_med np18* np19*_cgoodman_cz_pc np20*_cgoodman_cz_pc shift_share_base diff*_cgoodman_cz_pc
/*
...
forv i=6/16{
	if `i'!=4{
		...
		//ivreg2 n`y'_cgoodman_cz_pc (GM_raw_pp = shift_share_base) `b_controls' `extra_controls' [aw=popc1940], r
		reg n`y'_cgoodman_cz_pc GM_raw_pp `b_controls' `extra_controls' [aw = popc1940], r
		local b = e(b)[1,1]
...
keep cz growth19* growth20* decomp19* decomp20* popc1940 GM_raw_pp `b_controls' `extra_controls'  n19*_cgoodman_cz_pc n20*_cgoodman_cz_pc  np19*_cgoodman_cz_pc np20*_cgoodman_cz_pc n_cgoodman_cz_pc diff*_cgoodman_cz_pc
...
ssaggregate growth* decomp*_cgoodman_cz_pc n*_cgoodman_cz_pc GM_raw_pp  diff*_cgoodman_cz_pc [aw=popc1940], n("origin_fips") l(cz) sfile("$INTDATA/ssaggregate_prep/shares_base.dta") controls("`b_controls' `extra_controls'") s(share)
...
forv i=6/16{
	if `i'!=4{
		...
		ivreg2 diff`y'_cgoodman_cz_pc (GM_raw_pp = shift) [aw=s_n]
		local b = e(b)[1,1]
		local sd = e(V)[1,1]^(0.5)
		replace b = `b' if _n == `i'
		replace ci_hi  = `b' + 1.96 * `sd' if _n == `i'
