...
local controls reg2 reg3 reg4 mfg_lfshare1940 cz_popdens1940 mean_income_1940 sumshare_base sumshare_base_quad
...
gen x_quad = GM_raw_pp_quad
gen z_quad = shift_share_base_quad
...
foreach outcome in cgoodman  spdist gen_muni totfrac schdist_ind{
	...
	// First stage 1
	reg x z z_quad `controls' [aw=popc1940] if !mi(n_`outcome'_cz_pc),r
	local b_fs1_x_`outcome' = string(e(b)[1,1],"%9.3f")
	local se_fs1_x_`outcome' = string(e(V)[1,1]^0.5,"%9.3f")
	local b_fs1_x_quad_`outcome' = string(e(b)[1,2],"%9.3f")
	local se_fs1_x_quad_`outcome' = string(e(V)[2,2]^0.5,"%9.3f")
	test z = 0
	local F1_`outcome' : di %6.2f r(F)


	// First stage 2
	reg x_quad z z_quad `controls' [aw=popc1940] if !mi(n_`outcome'_cz_pc),r
	local b_fs2_x_`outcome' = string(e(b)[1,1],"%9.3f")
	local se_fs2_x_`outcome' = string(e(V)[1,1]^0.5,"%9.3f")
	local b_fs2_x_quad_`outcome' = string(e(b)[1,2],"%9.3f")
	local se_fs2_x_quad_`outcome' = string(e(V)[2,2]^0.5,"%9.3f")
	test z_quad = 0
	local F2_`outcome' : di %6.2f r(F)
	
	// OLS 1940-70
	reg y x x_quad `controls' [aw = popc1940], r
	...
	preserve
		ssaggregate y x x_quad eps [aw=popc1940], n(origin_fips) s(share) sfilename("$INTDATA/ssaggregate_prep/shares_base.dta") l(cz) controls("`controls'") 
		rename (y x x_quad eps s_n) (y1 x1 x_quad1 eps1 s_n1)
		tempfile f1
		save `f1'
	restore
...
* First with original shares
	...
	* Now with interacted shares
		ssaggregate y x x_quad eps [aw=popc1940], n(origin_fips) s(share) sfilename("$INTDATA/ssaggregate_prep/shares_base_quad.dta") l(cz) controls("`controls'") 
		rename (y x x_quad eps s_n) (y2 x2 x_quad2 eps2 s_n2)
		...
		ren shift g
		merge 1:1 origin_fips using "$INTDATA/ssaggregate_prep/shock_instrument_base_quad.dta", keep(3) nogen
		ren shift g_quad
		...
		* We use the output of ssaggregate: x_tilde1=s_n1*x1, and similar for all other variables
		...
		gen sxg12 = s_n1 * x_quad1 * g_res1 
		gen sxg21 = s_n2 * x2 * g_res2
		gen sxg22 = s_n2 * x_quad2 * g_res2 
		...
		* Compute the mean of the sandwich variance formula, equation (9) in the practical guide
		...
		local b_iv70_x_quad_`outcome' = string(b[2,1], "%9.3f")
		local se_iv70_x_`outcome' = string(V[1,1]^0.5, "%9.3f")
		local se_iv70_x_quad_`outcome' = string(V[2,2]^0.5, "%9.3f")
	restore
	// Long differences 
	...
	local b_ols10_x_quad_`outcome' = string(e(b)[1,2],"%9.3f")
	local se_ols10_x_`outcome' =string( e(V)[1,1]^0.5,"%9.3f")
	local se_ols10_x_quad_`outcome' =string( e(V)[2,2]^0.5,"%9.3f")
	...
	* Now with interacted shares
		...
		* Compute the mean of the sandwich variance formula, equation (9) in the practical guide
		...
		mat list M
				...
				mat invOmega = inv(Omega)
		mat list invOmega
		local b_iv10_x_`outcome' = string(b[1,1], "%9.3f")
		local b_iv10_x_quad_`outcome' = string(b[2,1], "%9.3f")
		local se_iv10_x_`outcome' = string(V[1,1]^0.5, "%9.3f")
		local se_iv10_x_quad_`outcome' = string(V[2,2]^0.5, "%9.3f")
	restore
}

foreach m in fs1 fs2 ols70 ols10 iv70 iv10{
	foreach v in x x_quad{
		foreach outcome in cgoodman  spdist gen_muni totfrac schdist_ind{
...
file open fh using "$TABS/quad_table.tex", write replace
...
file write fh  "$\widehat{GM}^2$ &    `b_fs1_x_quad_cgoodman' &    `b_fs1_x_quad_gen_muni' &    `b_fs1_x_quad_schdist_ind' &    `b_fs1_x_quad_spdist' &    `b_fs1_x_quad_totfrac' \\" _n
file write fh "                &  (`se_fs1_x_quad_cgoodman')  &  (`se_fs1_x_quad_gen_muni')  &  (`se_fs1_x_quad_schdist_ind')  &  (`se_fs1_x_quad_spdist')  &  (`se_fs1_x_quad_totfrac')  \\" _n
file write fh "\cmidrule(lr){1-6}" _n
...
file write fh  "$\widehat{GM}^2$ .  &    `b_fs2_x_quad_cgoodman' &    `b_fs2_x_quad_gen_muni' &    `b_fs2_x_quad_schdist_ind' &    `b_fs2_x_quad_spdist' &    `b_fs2_x_quad_totfrac' \\" _n
file write fh "                &  (`se_fs2_x_quad_cgoodman')  &  (`se_fs2_x_quad_gen_muni')  &  (`se_fs2_x_quad_schdist_ind')  &  (`se_fs2_x_quad_spdist')  &  (`se_fs2_x_quad_totfrac')  \\" _n
file write fh "\cmidrule(lr){1-6}" _n
file write fh "\cmidrule(lr){1-6}" _n
...
file write fh  "GM$^2$  &    `b_ols70_x_quad_cgoodman' &    `b_ols70_x_quad_gen_muni' &    `b_ols70_x_quad_schdist_ind' &    `b_ols70_x_quad_spdist' &    `b_ols70_x_quad_totfrac' \\" _n
file write fh "                &  (`se_ols70_x_quad_cgoodman')  &  (`se_ols70_x_quad_gen_muni')  &  (`se_ols70_x_quad_schdist_ind')  &  (`se_ols70_x_quad_spdist')  &  (`se_ols70_x_quad_totfrac')  \\" _n
file write fh "\cmidrule(lr){1-6}" _n
...
file write fh  "GM$^2$  &    `b_iv70_x_quad_cgoodman' &    `b_iv70_x_quad_gen_muni' &    `b_iv70_x_quad_schdist_ind' &    `b_iv70_x_quad_spdist' &    `b_iv70_x_quad_totfrac' \\" _n
file write fh "                &  (`se_iv70_x_quad_cgoodman')  &  (`se_iv70_x_quad_gen_muni')  &  (`se_iv70_x_quad_schdist_ind')  &  (`se_iv70_x_quad_spdist')  &  (`se_iv70_x_quad_totfrac')  \\" _n
file write fh "\cmidrule(lr){1-6}" _n
...
file write fh  "GM$^2$  &    `b_ols10_x_quad_cgoodman' &    `b_ols10_x_quad_gen_muni' &    `b_ols10_x_quad_schdist_ind' &    `b_ols10_x_quad_spdist' &    `b_ols10_x_quad_totfrac' \\" _n
file write fh "                &  (`se_ols10_x_quad_cgoodman')  &  (`se_ols10_x_quad_gen_muni')  &  (`se_ols10_x_quad_schdist_ind')  &  (`se_ols10_x_quad_spdist')  &  (`se_ols10_x_quad_totfrac')  \\" _n
file write fh "\cmidrule(lr){1-6}" _n
...
file write fh  "GM$^2$  &    `b_iv10_x_quad_cgoodman' &    `b_iv10_x_quad_gen_muni' &    `b_iv10_x_quad_schdist_ind' &    `b_iv10_x_quad_spdist' &    `b_iv10_x_quad_totfrac' \\" _n
file write fh "                &  (`se_iv10_x_quad_cgoodman')  &  (`se_iv10_x_quad_gen_muni')  &  (`se_iv10_x_quad_schdist_ind')  &  (`se_iv10_x_quad_spdist')  &  (`se_iv10_x_quad_totfrac')  \\" _n
file write fh "\cmidrule(lr){1-6}" _n
