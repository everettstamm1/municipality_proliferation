...
foreach outcome in cgoodman schdist_ind gen_town spdist gen_muni{
	su n1_`outcome'_cz_pc [aw=popc1940]
	local dv : di %6.2f r(mean)
	...
	// OLS
	eststo ols_`outcome' : reg n1_`outcome'_cz_pc GM_raw_pp `b_controls' [aw = popc1940], r
	
	// RF
	eststo rf_`outcome' : reg n1_`outcome'_cz_pc GM_hat_raw_pp `b_controls' [aw = popc1940], r
	
	// 2SLS 
	eststo iv_`outcome' : ivreg2 n1_`outcome'_cz_pc (GM_raw_pp = GM_hat_raw_pp) `b_controls' [aw = popc1940], r
		estadd local Fs = `F'
		estadd local dep_var = `dv'
...
// Panel A: First Stage
esttab fs_cgoodman fs_gen_muni fs_schdist_ind fs_gen_town fs_spdist  ///
	using "$TABS/final/main_effect_1.tex", ///
	replace se booktabs noconstant noobs compress frag label nomtitles nonum ///
	b(%03.2f) se(%03.2f) ///
	starlevels( * 0.10 ** 0.05 *** 0.01) ///
	posthead("&\multicolumn{1}{c}{C. Goodman}&\multicolumn{4}{c}{Census of Governments}\\\cmidrule(lr){2-2}\cmidrule(lr){3-6}" ///
