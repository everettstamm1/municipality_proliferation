...
foreach outcome in cgoodman schdist_ind gen_subcounty spdist{
	...
	if !_rc{
	qui parmby "reg n_`outcome'_cz_pcc GM_hat_raw_r`i' reg2 reg3 reg4 [aw=popc1940], r", lab saving(`"vr`i'`outcome'"', replace) idn(`i') ids(vr) ylabel rename(idn vrsn) level(95 99)
	}
	}
	...
	forval i=1/1000 {
	capture append using "vr`i'cgoodman"
	}
	...
	graph export "$FIGS/exogeneity_tests/D17_placebo_`outcome'_urban.png", replace 	
}