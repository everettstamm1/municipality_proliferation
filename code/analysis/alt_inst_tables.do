...
foreach inst in GM_hat_raw_pp GM_7r_hat_raw_pp GM_r_hat_raw_pp GM_1940_hat_raw_pp {
	...
	foreach outcome in cgoodman schdist_ind spdist gen_subcounty gen_town{
			...
			eststo ols : reg n_`outcome'_cz_pcc GM_raw_pp `b_controls' [aw=popc1940], r
			eststo rf : reg n_`outcome'_cz_pcc `inst' `b_controls' [aw=popc1940], r
			eststo tsls : ivreg2 n_`outcome'_cz_pcc (GM_raw_pp = `inst') `b_controls' [aw=popc1940], r
...
foreach w in wt_instmig_avg_pp GM_8_hat_raw_pp{
	foreach outcome in cgoodman schdist_ind spdist gen_subcounty gen_town{
		...
		local F : di %6.3f `r(F)'
		estadd local Fstat = `F'
		eststo ols : reg n_`outcome'_cz_pcc GM_raw_pp `b_controls' `w' [aw=popc1940], r
		eststo rf : reg n_`outcome'_cz_pcc GM_hat_raw_pp `b_controls' `w' [aw=popc1940], r
		eststo tsls : ivreg2 n_`outcome'_cz_pcc (GM_raw_pp = GM_hat_raw_pp) `b_controls' `w' [aw=popc1940], r
