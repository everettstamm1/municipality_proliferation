			...
			foreach level in county{
				...
				forv pc=0/1{
					foreach ds in schdist_ind all_local all_local_nosch cgoodman{
						...
						global C6 base_muni_`level'1940 reg2 reg3 reg4 blackmig3539_share1940
					...
					*/
						...
						forv lag = 0/0{
							...
							forv i=3/4{
								...
								savegraph("$FIGS/binscatter/binscatter_ctrls`i'_stacked_L`lag'_`ds'_`pc'_`level'_`x'_`drop'_`inst'.pdf") replace
								...
								esttab 	fs ///
												...
												rf	///
												tsls ///
												using "`filepath'/table_2_ctrls`i'_stacked_L`lag'_`ds'_`pc'_`level'_`x'_`drop'_`inst'.tex", ///
												replace label se booktabs num noconstant ///
												starlevels( * 0.10 ** 0.05 *** 0.01) ///
												stats(Fstat Rsquared dep_mean N, labels( ///
												"F-Stat"	///
