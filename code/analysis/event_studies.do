...
forv i=1/11{
	...
	g n_cgoodman_cz_`y' = b_cgoodman_cz`y' - b_cgoodman_cz`y1'
}
		
		
keep cz cz_name growth* decomp*  pop* popc1940 GM_raw_pp `b_controls' `extra_controls'  n19*_cgoodman_cz_pc n20*_cgoodman_cz_pc above_x_med  np19*_cgoodman_cz_pc np20*_cgoodman_cz_pc shift_share_base GM_hat_raw
/*
...
forv i=6/13{
	if `i'!=4{
		...
		di "`y'"
		reg n`y'_cgoodman_cz_pc shift_share_base `b_controls' [aw=pop1940] if !inlist(cz,34901,35001,35100), r
		local b = e(b)[1,1]
...
twoway 	(scatter b year, color(black)) ///
				...
				xlabel(1910(10)1980, val angle(45)) ///
				yline(0, lc(red) lp(dash)) ///
				legend(off) ytitle("Change in Municipalities P.C.")
				
				g diff = n1920_cgoodman_cz_pc - n1910_cgoodman_cz_pc
			
keep cz growth19* growth20* decomp19* decomp20* GM_raw_pp GM_hat_raw popc19* `b_controls' `extra_controls'  n19*_cgoodman_cz_pc n20*_cgoodman_cz_pc  np19*_cgoodman_cz_pc np20*_cgoodman_cz_pc 
