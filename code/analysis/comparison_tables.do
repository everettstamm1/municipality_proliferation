...
keep fips decade blackmig3539_share GM GM_hat reg* base_muni_county_L0 mfg_lfshare n_muni_county_L0

keep if decade == 1940
drop if GM==. | fips == . | reg3 == 1 
sort decade blackmig3539_share

order decade fips blackmig3539_share GM GM_hat

xtile blackmig_bin = blackmig3539_share if decade==1940, nq(10)
...
order decade blackmig3539_share blackmig_bin reg GM 
...
merge 1:1 fips decade using "$CLEANDATA/county_schdist_ind_stacked_og.dta", keep(3) nogen
drop *_L1 *_L2
...
foreach var of varlist GM GM_hat blackmig3539_share mfg_lfshare base* new* countypop{
...
/*
foreach var of varlist GM GM_hat v2_blackmig3539_share base* new*{
...
foreach i in `regbin'{
	...
	forv j=0/1{
		preserve 
			...
			qui estpost tabstat blackmig3539_share GM GM_hat base* new* mfg_lfshare countypop, ///
			statistics(mean) ///
