...
keep cz grow* pop* popc1940 GM_raw_pp `b_controls' `extra_controls'   above_x_med shift_share_base sumshare_base region b*_cgoodman_cz_pc
rename b*_cgoodman_cz_pc y*
...
// Run TWFE
reghdfe y ib1940.year##c.shift_share_base , absorb(cz year) vce(cluster cz)
...
forv y=1910(10)2010{
	if `y'==1940{
		...
		local name = "`y'.year#c.shift_share_base"
		local b = e(b)["y1","`name'"] 
...
graph export "$FIGS/RR2/twfe_main.png", as(png) replace




reghdfe y ib1940.year##c.shift_share_base ib1940.year##c.sumshare_base, absorb(cz year) vce(cluster cz)
...
graph export "$FIGS/RR2/twfe_sumshare_base.png", as(png) replace
...
graph export "$FIGS/RR2/twfe_popgrowth.png", as(png) replace
...
graph export "$FIGS/RR2/twfe_regions.png", as(png) replace
